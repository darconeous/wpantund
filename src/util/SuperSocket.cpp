	...
	:UnixSocket(-1, false), mPath(path)
{
	...
	if (0 > fd) {
		...
		throw SocketWrapperError("Unable to open socket");
	}
	...
	if ( (SUPER_SOCKET_TYPE_DEVICE == get_super_socket_type_from_path(path.c_str()))
	  && (flock(fd, LOCK_EX|LOCK_NB) < 0)
		...
		if (EWOULDBLOCK == errno) {
			...
			throw SocketWrapperError("Socket is locked by another process");
		}
	}
}
...
SuperSocket::reset()
{
	...
	if (mFDRead < 0) {
		...
		throw SocketWrapperError("Unable to reopen socket");
	}
